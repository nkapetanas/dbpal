class TS_Builder:
    ...
    def __init__(self, ppdb_path="/home/user/DBPal/ppdb/l-lexical"):
        self.output_prefix_ = "./"
    ...
    def create_sql_nl(self):
        ...
        nl_fields = {"id": "ids", "first_name": "name", "last_name": "surname", "diagnosis": "disease",
                     "gender": "gender", "length_of_stay": "hospilization period", "age": "age"}
        annot = {"id": "@ID", "first_name": "@FIRST_NAME", "last_name": "@LAST_NAME", "diagnosis": "@DIAGNOSIS",
                 "gender": "@GENDER", "length_of_stay": "@LENGTH_OF_STAY", "age": "@AGE"}
    ...
    def paraphrase(self, sql_input, nl_input, file_output):
        ...
        for i in range(0, len(sql_statements)):
            ...
            paraphrases = [self.lemmatize(lemmatizer, NL1, stop_words),
                           self.lemmatize(lemmatizer, NL2, stop_words)]  # a list of the nl_statements paraphrases
                ...
                if word not in stop_words:
                    ...
                    if len(synonyms) > 0:
                        for synonym in synonyms:
                            lemmatized_paraphrase = self.lemmatize(lemmatizer, NL1.replace(word, synonym[0]),
                                                                   stop_words)
            ...
            for word in wordlist_2:
                if word not in stop_words:
                    ...
                    if len(synonyms) > 0:
                        for synonym in synonyms:
                            lemmatized_paraphrase = self.lemmatize(lemmatizer, NL2.replace(word, synonym[0]),
                                                                   stop_words)
                            paraphrases.append(lemmatized_paraphrase)
        ...
        filename = "/media/user/tsdisk/" + file_output
    ...
    def merge(self):
        f = open("/media/user/tsdisk/paraphrased_nl_statements.txt", "r")
        f2 = open("/media/user/tsdisk/paraphrased_nlaggr_statements.txt", "r")
        merged = open("/media/user/tsdisk/training_set.txt", "w")
    ...
    def build(self):
        self.create_sql_nl()
        self.paraphrase(self.sql_statements_, self.nl_statements_, "paraphrased_nl_statements")
        self.paraphrase(self.sqlaggr_, self.nlaggr_, "paraphrased_nlaggr_statements")
